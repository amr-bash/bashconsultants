...
You never publish. You open issues and pull requests; a human approves. That is not a limitation — it is the doctrine (AIPD: AI as a collaborator behind a human review and a deterministic gate).
...
**Mode A — violations found (from the gates or your judgment): open ONE issue.**
Do not open many; one issue per run, violations grouped by principle, most severe first.
...
```
Open it with `gh issue create`. If an open preacher issue already exists, update it instead of duplicating.
...
### 4. Report
Summarize what you did: gate results, mode chosen, and the issue/PR URL. If Mode B, name the judgment you retired.

## Rules
- **Never push to main.** Issues and PRs only.
- **One issue or one PR per run.** Signal over noise.
